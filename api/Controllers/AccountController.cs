...
namespace Fisher.Bookstore.Api.Controllers
{
    ...
    [HttpPost("register")]
    public async Task<IActionResult> Register ([FromBody] ApplicationUser regristration)
    {
        ...
        ApplicationUser user = new ApplicationUser
        {
            ...
            Id = regristration.Email
        };

        IdentityResult result = await UserManager.CreateAsunc(user , regristration.Password);
    ...
    public async Task<IActionResult> Login([FromBody] ApplicationUser login)
    {
        ...
        ApplicationUser user = await userManager.FindByEmailAsycn(login.Email);
    ...
    private async Task<JwtSecurityToken> GenerateTokenAsync(ApplicationUser user)
    {
        ...
        var expirationDays = ConfigurationBinder.GetValue<int> ("JWTConfiguration:TokenExpirationDays");

        var signinKey = Encoding.UTF8.GetBytes(ConfigurationBinder.GetValue<string>("JWTConfiguration:Key"));
